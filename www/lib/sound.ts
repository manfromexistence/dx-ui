    ...
    const self = {
        volume: (vol?: number): any => {
            if (vol !== undefined && vol >= 0 && vol <= 1) {
                state._volume = vol;
                if (state._muted || !state.ctx) return self;
                if (state.usingWebAudio && state.masterGain) {
        ...
        codecs: (ext: string): boolean => state._codecs[ext.replace(/^x-/, "")],
        _setup: () => {
...
const createSound = (o: SoundOptions) => {
    ...
    const soundState = {
        ...
        _src: typeof o.src === "string" ? [o.src] : (o.src as string[]),
    ...
    const soundInstance: any = {
        ...
        rate: (rate?: number, id?: PlaybackId): any => {
            ...
            for (const playbackId of ids) {
                ...
                if (playback && DxSound._state.ctx) {
                    ...
                    const duration = (soundState._sprite[playback._sprite]?.[1] / 1000 || soundState._duration) - seek;
                    const timeout = (duration * 1000) / Math.abs(playback._rate);
        ...
        state: (): SoundState => soundState._state,
        unload: () => {
            ...
            const activeSrc = soundState._src[0];
            const remCache = !DxSound._state._sounds.some((sound) => sound._src[0] === activeSrc);
            if (cache[activeSrc] && remCache) {
                delete cache[activeSrc];
            }
        ...
        once: (event: string, fn: Function, id?: PlaybackId) => soundInstance.on(event, fn, id, true),
        ...
        _playbackById: (id: PlaybackId): any | null => soundState._playbacks.find((p) => p._id === id) || null,
        ...
        _getPlaybackIds: (id?: PlaybackId): PlaybackId[] => (id === undefined ? soundState._playbacks.map((p) => p._id) : [id]),
        _refreshBuffer: (playback: any) => {
            ...
            node.bufferSource.buffer = cache[soundState._src[0]];
        ...
        _decodeAudioData: (arraybuffer: ArrayBuffer) => {
            if (!DxSound._state.ctx) return;
            DxSound._state.ctx.decodeAudioData(
                arraybuffer,
                (buffer) => {
                    if (buffer && soundState._playbacks.length > 0) {
                        cache[soundState._src[0]] = buffer;
...
const createPlayback = (sound: Sound) => {
    ...
    const create = () => {
        ...
        if (parent._webAudio && DxSound._state.ctx) {
            ...
            (playback._node as HTMLAudioElement).src = parent._src[0];
            const preloadValue = parent._preload;
            (playback._node as HTMLAudioElement).preload = preloadValue === true ? "auto" : preloadValue === false ? "none" : preloadValue;
...
const useSoundStore = create<SoundStore>((set) => ({
    playing: new Set(),
    addPlaying: (id) => set((state) => ({ playing: new Set(state.playing).add(id) })),
    removePlaying: (id) => set((state) => {
...
export function useSound(src: string | string[], options: UseSoundOptions = {}): UseSoundTuple {
    const { id, volume = 1, rate: playbackRate = 1, soundEnabled = true, loop = false, interrupt = false, ...rest } = options;
    ...
    const stop = useCallback((id?: PlaybackId) => {
        if (soundRef.current) soundRef.current.stop(id);
    }, []);
    ...
    useEffect(() => {
        if (soundRef.current) soundRef.current.loop(loop);
    }, [loop]);
    useEffect(() => {
