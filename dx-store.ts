import {
  ...
  export function useStore<S extends StoreApi<unknown>>(api: S): ExtractStore<S>;
  ...
  const createImplReact = <T>(createStoreFn: StoreInitializer<T, [], []>) => {
    ...
    const persistStorage: PersistStorage<S> = {
      getItem: (name) => {
        const parse = (str: string | null) =>
          str === null ? null : (JSON.parse(str, options?.reviver) as StorageValue<S>);
